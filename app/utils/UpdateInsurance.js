...
const UpdateInsurance = ({open,handleClose,sentModalData}) => {
    ...
    useEffect(() => {{sentModalData[1] == 1 ? setAdminAction(1) : setAdminAction(0) }}, [sentModalData])
...
// let image22 = localStorage.getItem('img')
    ...
    return (
        ...
        >
          ...
          {sentModalData[1] == 1 ? <></> : <input defaultValue={fileName} {...register("pdfContratAssurance")} type="hidden"/> }
           ...
           <input defaultValue="" {...register("prixAssur" , { required: true })} type="Number"  step="0.00001" onChange={() => setFileSelect(1)}/>

           {errors?.prixAssur?.type === "required" && <p className='erroeMassage'>Prix Assurance est requis</p>}
          ...
          {errors?.operation?.type === "required" && <p className='erroeMassage'>operation est requis</p>}

           {/* <label>packassur_id:</label> */}
           <input defaultValue={sentModalData?.packassurance?.id || sentModalData[0]?.packassurance?.id} {...register("packassur_id" )} type="hidden"/>
           {/* <label>user_id:</label> */}
           <input defaultValue={sentModalData?.user?.id || sentModalData[0]?.user?.id} {...register("user_id")} type="hidden"/>
           {/* <label>idContrat:</label> */}
           <input defaultValue={sentModalData?.id || sentModalData[0]?.id} {...register("idContrat")} type="hidden"/>
         ...
         <input type="reset" value="réinitialiser" onClick={() => setFileSelect(0)}/>
           ...
           {FileSelect&&FileSelect1 ? <Box sx={{ mt: 5 }}><input type="submit" value="nous faire parvenir"/></Box>: <Box sx={{ mt: 5 }}>
          ...
          <input defaultValue="" type="file" {...register("document")}  webkitdirectory accept="application/pdf"  onChange={() => setpdfFileSelect(1)} />
          {errors?.document?.type === "required" && <p className='erroeMassage'>Sélectionnez le document en PDF est requis</p>}
   
          <Box sx={{ mt: 5 }}>

          <input type="reset" value="Réinitialiser" onClick={() => setpdfFileSelect(0)}/>
          </Box>


          {pdfFileSelect?<Box sx={{ mt: 5 }}>
