...
function ThemeWrapper(props) {
  let localModeStatus = localStorage.getItem('modeStatus');
  const [progress, setProgress] = useState(0);
  const [theme, setTheme] = useState(
    // eslint-disable-next-line
    createTheme(applicationTheme(props.color, localModeStatus ?localModeStatus : props.mode, props.direction))
  );
  ...
  const handleChangeMode = mode => { // eslint-disable-line
    ...
    setTheme(
      createTheme(
        applicationTheme(color,localModeStatus ? localModeStatus : mode  )
      )
    );
    changeMode(localModeStatus ? localModeStatus : mode);
  };
